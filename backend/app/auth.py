...
# One .env, at backend/ — next to run.sh, where you would look for it. The
# repo root is checked second so a single .env can serve backend/ and any
# sibling tooling.
...
def configure() -> str:
    ...
    Also resolves both model names into NITYAM_RESOLVED_LIVE_MODEL and
    NITYAM_RESOLVED_REASONING_MODEL, because on Vertex the Live API wants a
    full resource path and the project id has to be read *before* express mode
    removes it from the environment. Order is load-bearing.
    """
    ...
    os.environ["NITYAM_RESOLVED_LIVE_MODEL"] = resolve_model(
        mode, os.getenv("NITYAM_LIVE_MODEL", "").strip() or config.LIVE_MODEL
    )
    # Deliberately NOT expanded: the `1007 Invalid resource field value` that
    # forced the absolute path is specific to the Live surface. Ordinary
    # generate_content accepts a bare id in express mode (confirmed by shruti
    # and by the adk sub-module's own probe), and a bare id is one less thing
    # to get wrong.
    os.environ["NITYAM_RESOLVED_REASONING_MODEL"] = (
...
def resolve_model(mode: str, model: str) -> str:
    ...
    if not model:
        raise SystemExit("no model id given to resolve_model()")
...
def describe() -> str:
    ...
    live = os.getenv("NITYAM_RESOLVED_LIVE_MODEL") or config.LIVE_MODEL
    brain = os.getenv("NITYAM_RESOLVED_REASONING_MODEL") or config.REASONING_MODEL
    short = f"{live.rsplit('/', 1)[-1]} + {brain.rsplit('/', 1)[-1]}"
    if mode == "mock":
...
async def _probe_live(mode: str, model: str, key: str, project: str, location: str) -> bool:
    ...
    # Imported here, not borrowed from preflight()'s local scope: this function
    # is also called directly (by scripts, and when probing candidate model
    # ids), and relying on the caller's import raised NameError *after* a
    # successful model lookup — which read exactly like the model being
    # unavailable. It cost a wrong conclusion once already.
    from google import genai
