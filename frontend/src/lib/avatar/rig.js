/* Ported verbatim from sub_modules_examples/avatar/runtime/rig.js.
 *
...
(function (NS) {
  ...
  /* Halves the visible neck. The reference is a big head on a short neck, and
     the ported original ran the neck from y=208 to the collar at y=266. */
  var NECK_TRIM = 20;
  NS.DESIGN = { w: W, h: H };
  ...
  function drawBody(ctx, p) {
    ...
    /* The neck's exposed length is the gap between the chin and the collar,
       so the whole torso is lifted rather than the neck path being redrawn.
       Anything pinned to the bottom edge of the frame is pushed down by the
       same amount so the bust still fills it. */
    ctx.save();
    ctx.translate(0, -NECK_TRIM);
    ...
    ctx.bezierCurveTo(272, 316, 282, 328, 288, 369);
    ctx.lineTo(12, 369);
    ...
    ctx.beginPath(); ctx.moveTo(150, 302); ctx.lineTo(150, 369); ctx.stroke();
    ctx.fillStyle = C.button;
    [347, 365].forEach(function (by) {
    ...
    ctx.fillStyle = cs; ctx.fillRect(6, 244, 288, 129);
    ctx.restore();
    ctx.restore();
  }
