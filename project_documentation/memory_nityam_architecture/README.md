...
4a. **ECHO's Gemini-based transcription was swapped to local `faster-whisper`, tested against real audio on two different model sizes, and reverted back to Gemini after both showed real quality regressions.** Full sequence, each step verified against real audio, not mocks:
    ...
    - **Still open, unresolved:** ECHO's original reliability gap (non-deterministic timestamps and content across identical repeated Gemini calls, confirmed empirically — see this entry's history) was never fixed; going back to Gemini reintroduces it, and the dollar-cost concern that motivated trying Whisper in the first place is real and still unaddressed. Any future attempt at a local-model alternative should test fidelity on real accented/code-switched audio before adopting it, the same way this attempt was actually tested rather than assumed to work.
