...
def transcribe_audio(client, audio_path: str, recording_id: str) -> list[Utterance]:
    ...
    # to run under either.
    #
    # Reverted from a local faster-whisper model back to Gemini: real-audio
    # testing on two different videos showed Whisper (both large-v3 and
    # small) badly mistranscribing accented/code-switched openings —
    # phonetic Devanagari transliteration of English terms, and in one case
    # outright hallucinated gibberish across roughly the first third of a
    # 26-minute lecture. Gemini's prompt-based fidelity (explicit script
    # preservation rules below) held up better on the same content. ECHO's
    # own separate reliability gap (non-deterministic timestamps across
    # identical repeated calls) remains open — see
    # memory_nityam_architecture/README.md gap 4a.
