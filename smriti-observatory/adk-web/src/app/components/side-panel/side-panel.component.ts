...
export class SidePanelComponent implements AfterViewInit, OnInit {
  ...
  // The SMRITI memory layer's own student_id (app/agents/tutor_agent.py
  // seeds it into session state, defaulting to "demo_student") -- not
  // necessarily the same string as ADK's own userId, so read it from the
  // session's actual state when present rather than assuming the two are
  // interchangeable.
  studentId = computed(
      () => (this.currentSessionState()?.['student_id'] as string | undefined) ??
          this.userId());
